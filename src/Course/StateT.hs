...
liftFToStateValue :: (a -> b) -> (a, state) -> (b, state)
liftFToStateValue f = \(a, nextState) -> (f a, nextState)
...
instance Monad f => Applicative (StateT s f) where
  pure ::
    a
    -> StateT s f a
  pure x = StateT $ \initialState -> pure (x, initialState)
...
instance Functor f => Functor (OptionalT f) where
  f <$> optionalT = OptionalT $ (\optional -> f <$> optional) <$> (runOptionalT optionalT)
...
instance Monad f => Monad (OptionalT f) where
  ...
  fn =<< optionalT = OptionalT $ do
    optional <- runOptionalT optionalT
    onFull (runOptionalT . fn) optional
...
instance Applicative (Logger l) where
  pure = Logger Nil
  (Logger lf f) <*> (Logger l a) = Logger (lf ++ l) (f a)
...
instance Monad (Logger l) where
  f =<< (Logger la a) = let (Logger lb b) = f a in Logger (la ++ lb) b
...
log1 ::
  ...
  -> a
  -> Logger l a
log1 singleLog a = Logger (listh [singleLog]) a
